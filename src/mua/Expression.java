...
public class Expression {
    ...
    public static String getExpress(String expression) {
        //预格式化，解决--，++的问题
    ...
    public static String compute(String value1, String value2, String operator) throws Exception {
        switch (operator) {
            case "+":
                return Operator.add(new Value(1, value1), new Value(1, value2), null).getValue();
            case "-":
                return Operator.sub(new Value(1, value1), new Value(1, value2), null).getValue();
            case "*":
                return Operator.mul(new Value(1, value1), new Value(1, value2), null).getValue();
            case "/":
                return Operator.div(new Value(1, value1), new Value(1, value2), null).getValue();
            case "%":
                return Operator.mod(new Value(1, value1), new Value(1, value2), null).getValue();
            default:
    ...
    //将表达式转换为后缀表达式栈
    public static Stack<String> infix2Postfix(String expression) {
        ...
        while (currentPos < allChars.length) {
            //如果是运算符
            if (Interpreter.isOp(String.valueOf(allChars[currentPos]))) {
                ...
                String tempNum = "";
                while (tempCurrentPos < allChars.length && !Interpreter.isOp(String.valueOf(allChars[tempCurrentPos])))
                ...
                if (Interpreter.isNumber(tempNum))
                    result.push(tempNum);
